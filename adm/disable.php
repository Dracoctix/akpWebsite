                ...
                if ($infosUser = $testUser->fetch()) {
                    if(!$infosUser['rank']) {
                        if ($infosUser['active'] && !$infosUser['banned']) {
                            $desactivation = $bdd->prepare('UPDATE users SET banned = 1 WHERE iduser = :iduser');
                            $desactivation->execute(array("iduser" => $_GET['iduser'])); // On désactive l'utilisateur
                            ?>
                            <script>
                                alert("Le compte de <?php echo $infosUser['username']; ?> a correctement été désactivé.");
                        ...
                        ?>
                            <script>
                                alert("Le compte de <?php echo $infosUser['username']; ?> n'est pas actif.");
                                document.location.href = "index.php";
                            </script>
                            <?php
                        }
                        ...
                        ?>
                        <script>
                            alert("Le compte que vous tentez de désactiver est administrateur.");
                            document.location.href="index.php";
                        </script>
                        <?php
                    }
